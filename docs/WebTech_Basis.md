...
### バックアップ運用
- サーバーの故障や不正アクセス, 被災などに備えて Webサービスの根幹たるデータは**バックアップ**をとるべきである.
  - **バックアップデータ**の削除や更新, 取得を**バックアップ運用**という.
- バックアップの対象となるのは以下.
  - アプリケーションやコンテンツといったファイル：ファイルをコピーすることでバックアップができる.
  - データベースの中身：DBMS の機能を使用して, 別のサーバーなどに退避できる.
- バックアップの頻度や世代
  - バックアップから復旧する際にはバックアップ時点のデータに遡ってしまう.
  - したがって, 頻繁にバックアップを取ったり, 何世代かにわたってバックアップを取るとよい.

### ログ運用
- サーバーやネットワーク機器は動作中の状態変化や処理の内容を**ログファイル**としてテキストファイルに保管している.
  - **システムログ**：OS が発生したイベントやエラーを記録する.
  - **アプリケーションログ**：ミドルウェアなどのアプリケーションが動作履歴を記録する.
  - **アクセスログ**：Webサーバーが受けたリクエストを記録する. これを解析して, サービス向上につなげるのが**アクセス解析**である.
- 適切なタイミングでログファイルは整理する必要があり, その運用を**ログメンテナンス**という.
  - 1か月, 1週間など日ベースで切り分けることもあれば, ファイルサイズで切り分けることもある.
  - このような切り分けを**ログローテーション**, **ハウスキープ**という.

### Webサイトのパフォーマンス
- ユーザーが利用する際の満足度の1つとして機能している.
- パフォーマンスの指標.
  - **応答時間**：リクエストからレスポンスまでの時間.
  - **表示完了時間**：リクエスト送信から, Webページの表示が完了するまでの時間.
  - **ページ読み込み時間**：ページの読み込み開始から終了までの時間.
  - **可用性**：エラーなく Webサイトにアクセスできたかという確率.
- パフォーマンスを監視することでシステムを改善していくのが基本であり, 同時に CPU やメモリの利用率といった**サーバーの負荷状態**もモニタリングしておくことが, 特に可用性の改善につながる.

### 脆弱性診断
- セキュリティ確保のために**脆弱性対策**は必須である.
  - 使用しているミドルウェアやOSに時間経過で脆弱性が発見されることも少なくないため, 脆弱性データベースを定期的に確認しておくことが肝要となる.
- 自分で開発したサービスについては自分で脆弱性診断を行う必要があり, 疑似的にサイバー攻撃を受けてみる**ペネトレーションテスト**などが手法として挙げられる.